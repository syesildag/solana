...
| `MOMENTUM_LOCAL_IMPACT` | `false` | Local impact pre-gate: skip an entry/rotation candidate WITHOUT a Jupiter REST quote when the gRPC ingestion task's estimated price impact of a `MOMENTUM_TRADE_USDC`-sized buy (from live pool state; CP + Whirlpool pools only — DLMM's quote model is pure-linear, no signal) is fresh (<120s) and exceeds 2× `MOMENTUM_MAX_COST_BPS`. The local model ignores routing, so only an obviously-doomed candidate is skipped locally — anything closer is left to the authoritative Jupiter quote's cost gate. |
