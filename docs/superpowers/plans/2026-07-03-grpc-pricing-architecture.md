...
- Modify: `src/bin/portfolio_watcher.rs` (publish estimate in `apply_update` for CP + Whirlpool pools; needs `trade_usdc` — thread `cfg.momentum_trade_usdc` into the wired setup as a captured value; buy direction = quote→momentum, amount_in = `trade_usdc` in quote atomic units, ÷ `sol_usd` first when quote is SOL; dispatch by `w.pool.dex`: `raydium_amm::get_quote`/`saber::get_quote` for CP, `orca::get_quote` for Whirlpool — each returns `SwapQuote { price_impact, .. }` (fraction) → bps; skip DLMM (its `get_quote` is pure-linear, `price_impact` hardcoded 0.0 — no signal) and other kinds)
- **Amendment (discovered during recon):** `orca::get_quote`'s depth factor reads `pool.reserve_a/reserve_b` (vault balances), which the watcher does NOT subscribe for CL pools — reserves would stay 0 and impact would always read 0.0. Fix in `spawn_grpc_feed`: for `OrcaWhirlpool` wired pools, ALSO insert `vault_a`/`vault_b` into `acct_index` (as `Role::VaultA`/`Role::VaultB`, alongside `Role::State`) so seeding + the stream populate reserves. `apply_update` already handles those roles. A vault write also re-publishes the (unchanged) price with a fresh timestamp — semantically correct (vault write = trading activity). DLMM vaults are NOT added (no consumer).
