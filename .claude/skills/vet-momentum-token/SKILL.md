...
- **Liquid:** Birdeye liquidity ≥ **$0.44M** AND a vol/liq turnover ratio in the band
  **0.5 ≤ vol/liq ≤ 30** — depth to trade, plus turnover that proves it's actually traded
  (floor, anti-stale) but isn't churning suspiciously (cap, anti-wash; the same `30` the
  scanner uses). Checked **first** and short-circuits before the backfill+grid. Liquidity
  reads from Birdeye's cheap `/price?include_liquidity=true` (survives the CU quota that
  kills `token_overview`) so it stays on Birdeye's scale; 24h volume comes from
  GeckoTerminal. If liquidity can't be read from either source, the token is **REJECTED**
  (we don't add liquidity we couldn't confirm). Note Birdeye reads ~2–3× higher than
  GeckoTerminal/DexScreener — the $0.44M floor is calibrated to Birdeye.
...
All three must hold. Tune with `--min-liquidity`, `--min-vol-liq-ratio`,
`--max-vol-liq-ratio`, `--vol-floor`, `--min-trades`, `--days`. (The same band applies to
the live discovery scanner via `SCAN_MIN_LIQUIDITY` / `SCAN_MIN_RATIO` / `SCAN_MAX_RATIO`
in `scan_tokens.js`.)
