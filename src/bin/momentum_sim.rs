...
enum Command {
    ...
    MaxnCompare {
        ...
        /// above entry releases it (original behavior — too weak: +0.03% exempted a position
        /// that then fell 10.1%). Comma-separated list to sweep.
        #[arg(long, value_delimiter = ',', default_value = "0")]
        ...
        #[arg(long, default_value_t = 0.0)]
        crash_exit_k: f64,
        /// SIM EXPERIMENT: spike-TOP take-profit — sell a green position once price has risen
        /// k × σ(--vol-obs) (floored at MOMENTUM_SPIKE_EXIT_MIN_BPS) above its low over the last
        /// --crash-exit-obs observations; the upward mirror of --crash-exit-k. 0 = off.
        #[arg(long, default_value_t = 0.0)]
        spike_tp_k: f64,
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::MaxnCompare {
            ...
            crash_exit_obs, crash_exit_k, spike_tp_k, max_n,
        } => {
            let m = metric.parse::<RankMetric>().map_err(|e| anyhow::anyhow!("bad --metric: {e}"))?;
