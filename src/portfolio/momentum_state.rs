...
mod tests {
    ...
    fn entries_last_24h_counts_open_and_recent_closed() {
        ...
        state.trades.push(TradeRecord {
            ...
            exit_ts: now - 90_000,
            mint: "M".into(), symbol: "S".into(),
            entry_price_usd: 1.0, exit_price_usd: 1.1, peak_price_usd: 1.2,
            usdc_in: 50.0, usdc_out: 55.0, pnl_pct: 10.0,
            entry_sig: "a".into(), exit_sig: "b".into(), dry_run: true,
        });
    ...
    fn repair_peak_heals_invalid_and_keeps_valid() {
        ...
        p.repair_peak(0.0);
        assert!((p.peak_price_usd - 100.0).abs() < 1e-9, "peak floored at entry");
