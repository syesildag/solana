...
pub fn get_quote(pool: &Pool, amount_in: u64, a_to_b: bool) -> SwapQuote {
    ...
    let (amount_out, price_impact) = if pool.stable {
        ...
        // Per Meteora DAMM SDK: StableSwap invariant operates at ra/rb level directly.
        let out = crate::dex::stable_math::get_amount_out_damm(
            amount_in, ra, rb, amp, fee_bps, price_scale, a_to_b,
        );
        tracing::debug!(
            pool = &pool.id.to_string()[..8],
            a_to_b, amount_in, out,
            ra, rb, vpr = price_scale,
            rate = if amount_in > 0 { out as f64 / amount_in as f64 } else { 0.0 },
            "DAMM stable quote"
        );
